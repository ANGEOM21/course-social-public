<div>
  ...
  <div
        ...
        @else
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
            @foreach ($categories as $category)
              ...
              <div wire:key="category-{{ $category->id_category }}"
                class="card bg-base-100 shadow-sm hover:shadow-lg border border-base-300/80 transition-all duration-300 hover:-translate-y-1 group overflow-hidden">
                <figure class="aspect-video bg-base-200">
                  @if ($category->img_category)
                    <img src="{{ Storage::url($category->img_category) }}" alt="{{ $category->name_category }}"
                ...
                <div class="card-body p-4">
                  <h2 class="card-title text-base font-semibold group-hover:text-primary transition-colors truncate"
                  ...
                  <div class="card-actions justify-end mt-4 pt-3 border-t border-base-200">
                    <button wire:click="edit({{ $category->id_category }})"
                      class="btn btn-xs btn-outline btn-info hover:text-white"><i class="fa-solid fa-pencil"></i>
                      Edit
                    </button>
                    <button wire:click="delete({{ $category->id_category }})"
                      wire:confirm.prompt="Anda yakin ingin menghapus kategori '{{ $category->name_category }}'? Tindakan ini tidak dapat dibatalkan.\n\nKETIK 'HAPUS' UNTUK MELANJUTKAN.|HAPUS"
    ...
    @if ($categories->hasPages())
      <div class="mt-8 pt-6 border-t border-base-200 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="text-sm text-base-content/60">Menampilkan {{ $categories->firstItem() }} -
          {{ $categories->lastItem() }} dari {{ $categories->total() }} hasil</div>
  ...
  <x-modal wire:model="categoryModal" id="category-modal" :title="$isEdit ? 'Edit Kategori' : 'Buat Kategori Baru'" :subtitle="$isEdit ? 'Ubah informasi kategori yang dipilih.' : 'Tambahkan kategori baru ke dalam sistem.'" separator
    box-class="max-w-md">

    {{-- Loading overlay untuk form --}}
    <div wire:loading wire:target="save, img_category"
      class="absolute inset-0 bg-base-100/80 rounded-xl flex items-center justify-center z-10">
      <div class="text-center"><span class="loading loading-spinner loading-lg text-primary mb-2"></span>
        <p class="text-sm text-base-content/60"><span wire:loading wire:target="save">Menyimpan...</span><span
            wire:loading wire:target="img_category">Mengunggah...</span></p>
      </div>
    </div>
    ...
    <form wire:submit="save">
      <div class="space-y-7">
        <label class="form-control w-full">
          ...
          <input wire:model="name_category" type="text" placeholder="Contoh: Pemrograman Web"
            class="input input-bordered w-full focus:input-primary" autofocus />
          @error('name_category')
        ...
        <div x-data="{ isDragging: false }" class="w-full">
          ...
          <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
            @drop.prevent="isDragging = false; const files = $event.dataTransfer.files; if (files.length > 0) { $wire.upload('img_category', files[0]) }"
            :class="{ 'border-primary bg-primary/10': isDragging }"
            class="relative flex flex-col items-center justify-center w-full aspect-video border-2 border-dashed border-base-300 rounded-lg cursor-pointer transition-all duration-300">
            <div class="w-full h-full">
              @if ($img_category)
                <img src="{{ $img_category->temporaryUrl() }}" class="w-full h-full object-cover rounded-lg">
              @elseif ($existing_img_category)
                <img src="{{ Storage::url($existing_img_category) }}" class="w-full h-full object-cover rounded-lg">
              @else<div class="flex flex-col items-center justify-center h-full text-base-content/50"><i
                    class="fa-solid fa-camera text-4xl mb-3"></i>
                  <p class="text-sm font-semibold">Drag & drop atau klik di sini</p>
                  <p class="text-xs mt-1">PNG, JPG, WEBP (maks. 2MB)</p>
                </div>
              @endif
            </div>
            <input type="file" wire:model="img_category" class="hidden" x-ref="fileInput">
            <button type="button" @click="$refs.fileInput.click()"
              class="absolute inset-0 w-full h-full z-5"></button>
          </div>
          @error('img_category')
            <div class="label"><span class="label-text-alt text-error">{{ $message }}</span></div>
          @enderror
